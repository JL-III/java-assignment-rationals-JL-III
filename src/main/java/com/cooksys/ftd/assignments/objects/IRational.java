...
interface IRational {
	...
	default IRational add(IRational that) throws IllegalArgumentException {
		...
		//"this" is not needed here but helped me see the difference between "this" and "that". The variables probably aren't needed either - but reading the return construct would be messier without them
		int n1 = this.getNumerator();
		int d1 = this.getDenominator();
		int n2 = that.getNumerator();
		int d2 = that.getDenominator();
		//Though this is do-able without extra parenthesis, i think this helps readability
